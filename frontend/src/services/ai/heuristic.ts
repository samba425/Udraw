/**
 * Offline diagram generator used when the backend AI service is unavailable.
 * Turns a natural-language prompt into a linear flowchart specification so the
 * "AI generate" feature keeps working fully offline. Mirrors the backend
 * heuristic in `backend/src/ai/heuristic.ts`.
 * @module services/ai/heuristic
 */
...
/** Generate a flowchart specification from a prompt, entirely offline. */
export function generateHeuristicDiagram(prompt: string, maxNodes = 30): DiagramSpec {
  ...
  const nodes: DiagramNode[] = steps.map((step, i) => ({
    id: `n${i + 1}`,
    label: step.replace(/\s+/g, ' ').slice(0, 80),
