/**
 * Networking library: router, switch, firewall, server, cloud, laptop,
 * database, mobile, printer, access point.
 * @module shapes/libraries/network
 */
import { glyphIcon } from './builders';
import type { LibraryIcon } from '@/shapes/registry';

const NET_BLUE = '#1ba0d7';
const W = `fill="none" stroke="${NET_BLUE}" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"`;
const F = `fill="#e0f2fe" stroke="${NET_BLUE}" stroke-width="2.5"`;
...
export const NETWORK_ICONS: LibraryIcon[] = [
  ...
  glyphIcon('net.server', 'Server', 'Network', `<rect x="12" y="6" width="24" height="36" rx="2" ${F}/><circle cx="18" cy="13" r="1.6" fill="${NET_BLUE}"/><circle cx="18" cy="20" r="1.6" fill="${NET_BLUE}"/><path d="M24 13 h8 M24 20 h8" ${W}/>`, { size: { width: 56, height: 84 } }),
  ...
  glyphIcon('net.printer', 'Printer', 'Network', `<rect x="10" y="18" width="28" height="16" rx="2" ${F}/><rect x="16" y="8" width="16" height="10" ${F}/><rect x="16" y="30" width="16" height="10" fill="#fff" stroke="${NET_BLUE}" stroke-width="2"/>`, { size: { width: 72, height: 72 } }),
  glyphIcon('net.accesspoint', 'Access Point', 'Network', `<circle cx="24" cy="34" r="4" fill="${NET_BLUE}"/><path d="M16 26 a11 11 0 0 1 16 0 M12 21 a17 17 0 0 1 24 0" ${W}/>`, { size: { width: 72, height: 60 } }),
];
